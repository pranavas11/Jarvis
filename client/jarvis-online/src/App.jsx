...
function App() {
  ...
  const jarvisMessageIndex = useRef(-1);
  ...
  const getCurrentTime = () => {
    return new Date().toLocaleString("en-US", {
      // Note: change timeZone to reflect your actual location based on context
  ...
  useEffect(() => {
    ...
    const handleError = (event) => {
      ...
      // Set visualizer to idle only if jarvis isn't speaking
    ...
    const handleEnd = () => {
      ...
      if (
        ...
        jarvisMessageIndex.current = -1; // Reset index for next jarvis message
        if (socket.current?.connected) {
          socket.current.emit("send_transcribed_text", {
            transcript: processedTranscript,
          });
        }
        setStatusText("Waiting for Jarvis..."); // Update status
      } else {
  ...
  useEffect(() => {
    ...
    // Handles incoming text chunks for jarvis' response
    const handleTextChunk = (data) => {
      if (!data || !data.text) return; // Ignore empty chunks
      setMessages((prevMessages) => {
        const newMessages = [...prevMessages];
        // Check if the last message is from jarvis and append, or add new message
        if (
          jarvisMessageIndex.current !== -1 && // Ensure index is valid
          jarvisMessageIndex.current < newMessages.length && // Bounds check
          newMessages[jarvisMessageIndex.current]?.sender === "jarvis" // "ada"
        ) {
          newMessages[jarvisMessageIndex.current].text += data.text;
        } else {
          // Add a new message entry for jarvis
          newMessages.push({ sender: "jarvis", text: data.text });
          jarvisMessageIndex.current = newMessages.length - 1; // Update index
        }
        return newMessages;
      });
    };

    // Handles incoming audio chunks for jarvis' speech
  ...
  const handleSendText = useCallback(
    (text) => {
      if (text && socket.current?.connected) {
        ...
        jarvisMessageIndex.current = -1; // Reset jarvis message index
  ...
  return (
    <div className="app-container">
      {/* <h1>Jarvis AI</h1> */}
      <img src="/jarvis.jpg" alt="Jarvis AI" className="jarvis-logo" />
