# server/Jarvis_Online.py (Revised: Emits moved into functions)
import asyncio
import base64
import torch
...
VOICE_ID = 'OB0Jj6v9DGLLgz8dD57i' # 'pFZP5JQG7iQjIQuC4Bku'
CHANNELS = 1
...
# Which voices are actually available to this key?
# r = requests.get("https://api.elevenlabs.io/v1/voices", headers={"xi-api-key": ELEVENLABS_API_KEY})
...
class Jarvis:
    def __init__(self, socketio_instance=None, client_sid=None):
        # --- Initialization ---
        print("initializing Jarvis for web...")
        ...
        Your name is Jarvis. You have a joking and slighty flirty personality.
        I am your creator my name is Pranav, and you address him as "Pranav" and you also speak with an American accent.
    ...
    def _sync_Google_Search(self, query: str, num_results: int = 5) -> list:
        ...
        try:
            ...
            try:
                print("Retrying with basic search parameters...")
                results = list(Google_Search_sync(
                    term=query, 
                    num_results=min(num_results, 3),  # Fewer results for reliability
                    lang="en", 
                    timeout=15  # Even longer timeout for retry
                ))
    ...
    async def start_all_tasks(self):
        ...
        if not self.tasks:
            ...
            print(f"Jarvis Core Tasks started: {len(self.tasks)}")
        else:
            print("Jarvis tasks already running.")

    async def stop_all_tasks(self):
        print("Stopping Jarvis background tasks...")
