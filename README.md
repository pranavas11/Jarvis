...
1.  **Backend (`backend/app.py`, `backend/Jarvis_Online.py`):**
    ...
    - An `Jarvis` class instance (`Jarvis_Online.py`) encapsulates the core assistant logic.
...
2.  **Frontend (`frontend/src/App.jsx`, components):**
    ...
    - The `AiVisualizer` component changes appearance based on Jarvis's status (idle, listening, speaking).
